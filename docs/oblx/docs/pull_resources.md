...
The exome definition section covers exonic interval and CDS files for
downstream variant calling tools. In human and mouse mode the following files
can be found in this directory:

- `ref_cds.bed`: The CDS (coding sequence) intervals derived from the
  annotation GTF. CDS regions are merged. Based on the DeepVariant RNA-seq
